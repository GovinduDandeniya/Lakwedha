...
final orderPriceProvider = FutureProvider.family.autoDispose<Map<String, dynamic>, String>((ref, orderId) async {
  final dio = ref.watch(dioProvider);
  final response = await dio.get('/orders/$orderId/calculate-price');
...
class _PaymentSelectionScreenState extends ConsumerState<PaymentSelectionScreen> with TickerProviderStateMixin {
  String _selectedMethod = 'online'; // Assuming options are online, cod etc per backend enum
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      // Move order status strictly through the machine (approved -> processing)
      await dio.put('/orders/${widget.orderId}/status', data: {
        'status': 'processing',
        'reason': 'Payment successful via $_selectedMethod'
      });
  ...
  Widget build(BuildContext context) {
    final priceAsync = ref.watch(orderPriceProvider(widget.orderId));
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            priceAsync.when(
              data: (data) => _buildAmountCard(data['totalAmount'].toString()),
              loading: () => _buildShimmerAmountCard(),
              error: (err, stack) => const Text('Error loading price data', style: TextStyle(color: Colors.red)),
            ),
            ...
            priceAsync.when(
              data: (data) => _buildPayNowButton(data['totalAmount'].toString()),
              ...
              boxShadow: [
                BoxShadow(
                  color: AppTheme.earth.withOpacity(
                      0.2 + 0.08 * _pulseAnimation.value),
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  ...
                  Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(builder: (context) => ProviderScope(child: OrderTrackingScreen(orderId: widget.orderId)))
                  );
                },
