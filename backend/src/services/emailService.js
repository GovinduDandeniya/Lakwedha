 ...
 * @param {string} message - Email body (plain text; also rendered as HTML)
...
const sendEmail = async (to, subject, message) => {
  try {
    if (!transporter) {
      ...
      if (!initialized) {
        throw new Error('Email transporter is not initialized. Check EMAIL_USER and EMAIL_PASS in .env');
      }
    }

    if (!to || !to.includes('@')) throw new Error(`Invalid email address: ${to}`);
    if (!subject || subject.trim().length === 0) throw new Error('Email subject cannot be empty');
    if (!message || message.trim().length === 0) throw new Error('Email message body cannot be empty');
